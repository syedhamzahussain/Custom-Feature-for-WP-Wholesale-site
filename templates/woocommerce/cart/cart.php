	...
	<div class="cfws_cart_form">
		<form class="woocommerce-cart-form" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
			<table class="shop_table shop_table_responsive cart woocommerce-cart-form__contents" cellspacing="0">
				<thead>
					<tr>
						<th class="product-image"><?php esc_html_e( 'Image', 'woocommerce' ); ?></th>
					...
					foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
						...
						if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
							...
							<td class="product-price" data-title="<?php esc_attr_e( 'Price', 'woocommerce' ); ?>">
								<?php
								// if($count == 2)
								if(cfws_check_offered_price($cart_item)){
									// var_dump(cfws_check_offered_price($cart_item)); die();
									if(isset($cart_item['offered_price']) && !empty($cart_item['offered_price'])){

										echo  get_woocommerce_currency_symbol() . "<input type='number' step='0.01' name='" . 'cart[' . $cart_item_key . "][offered_price]' class='cfws_custom_cart_price' value='" . $cart_item['offered_price'] . "'/>";
									}
									else{
										echo  get_woocommerce_currency_symbol() . "<input type='number' step='0.01' name='" . 'cart[' . $cart_item_key . "][offered_price]' class='cfws_custom_cart_price' value='" . $_product->get_price() . "'/>";
									}
								}
								else{
									// var_dump('test'); die();
