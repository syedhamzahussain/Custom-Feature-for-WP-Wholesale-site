...
if ( ! class_exists( 'CFWS_ADMIN_PRODUCT' ) ) {
	...
	class CFWS_ADMIN_PRODUCT {
		...
		function cfws_save_custom_field_variations( $variation_id, $i ) {
			...
			if ( isset( $cost ) ) {
				update_post_meta( $variation_id, 'cfws_unit_cost', esc_attr( $cost ) );
			}
			if ( isset( $unit ) ) {
				update_post_meta( $variation_id, 'cfws_unit_quantity', esc_attr( $unit ) );
			}
		}
		...
		public function cfws_packages_tab_fields( $post_id ) {
			...
			if ( isset( $_POST['cfws_min_unit'] ) ) {
				...
				foreach ( $_POST['cfws_min_unit'] as $key => $value ) {
					$packages[] = array(
						'min'           => $value,
						'max'           => $_POST['cfws_max_unit'][ $key ],
						'discount_type' => $_POST['cfws_discount_type'][ $key ],
						'discount'      => $_POST['cfws_discount'][ $key ],
					);
				}
